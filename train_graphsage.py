...
# train_graphsage.py
# Full GraphSAGE training script with detailed inline comments,
# supports distribution, root_probability, regression modes,
# optional outlier analysis, plotting, temperature scaling, and K-Fold.
...
# # --------------------------------------------
# # wandb.init(project="graphsage_hyperparam_tuning", config=params)
# # params = wandb.config
...
#         underprediction_penalty (float): Multiplier for the loss when targets > preds.
#         low_target_penalty (float): Multiplier for the loss when targets < low_target_threshold.
#         low_target_threshold (float): The threshold defining a 'low' target value.

#     Returns:
#         torch.Tensor: The mean loss.
#     """
#     # 1. Calculate the basic squared error
#     diff = targets - preds
#     squared_error = diff ** 2
    
#     # Initialize a tensor of weights, all starting at 1.0 (standard MSE weight)
#     weights = torch.ones_like(targets, dtype=torch.float32)
    
#     # 2. Apply Underprediction Penalty
#     # Identify cases where the model underpredicts (targets > preds, so diff > 0)
#     underprediction_mask = diff > 0
#     # Increase the weight for underpredictions
#     weights[underprediction_mask] *= underprediction_penalty
    
#     # 3. Apply Low Target Value Penalty
#     # Identify cases where the target value is low
#     low_target_mask = targets < low_target_threshold
#     # Increase the weight for low target values
#     # We use '*' to compound the penalty if an underprediction also has a low target value.
...
# elif mode == "root_probability":
#     out_channels = 1
#     # loss_fn = lambda pred, true: simple_asymmetric_loss(pred, true, penalty=1.05)
#     loss_fn = lambda pred, true: asymmetric_weighted_loss(pred, true, 
#                                                         underprediction_penalty=2.0, 
#                                                         low_target_penalty=2.0, 
#                                                         low_target_threshold=0.0001)
#     # loss_fn = lambda pred, true: weighted_mse_loss(pred, true)
...
#         loss.backward()
#         # check prediction 
#         print(f"\n=== check predicton ----- very important -----{batch_idx} DEBUG ===")
...
#         metrics["avg_underpredict_error"] = np.mean(diff[under_mask]) if np.any(under_mask) else 0.0
#         metrics["mae"] = np.mean(np.abs(preds - trues))
#         metrics["rmse"] = np.sqrt(np.mean((preds - trues)**2))
#     else:
#         metrics["mae"] = np.mean(np.abs(preds - trues))
...
# # --------------------------------------------
# # Full training loop with optional K-Fold
# # --------------------------------------------
...
#             train_model(model, optimizer, early_stopping, train_loader, val_loader, test_loader, fold_idx=fold_idx)
#     else:
#         train_loader = DataLoader(train_set, batch_size=params['batch_size'], shuffle=True)
#         val_loader = DataLoader(val_set, batch_size=params['batch_size'])
#         test_loader = DataLoader(test_set, batch_size=params['batch_size'])
...
#     print(f"Starting training{' for fold ' + str(fold_idx) if fold_idx is not None else ''}...")
...
#         print(f"Epoch {epoch:03d} | Train Loss: {train_loss:.4f} | Val Loss: {metrics.get('loss', float('nan')):.4f} | "
...
#         # Step 1: initialize the scaler (only init_temp)
#         temp_scaler = TemperatureScaling(init_temp=1.0)

#         # Step 2: calibrate temperature on validation set
#         calibrated_temp = temp_scaler.calibrate(
#             model=model,           # trained GraphSAGE model
#             loader=val_loader,     # validation DataLoader
#             device=device,         # 'cuda' or 'cpu'
#             mode=config["mode"]    # "root_probability", "distribution", or "regression"
#         )
#         print(f"Learned temperature: {calibrated_temp:.4f}")

#         # Step 3: apply temperature scaling to test predictions (optional)
#         preds, targets = temp_scaler.apply(
#             model=model,
#             loader=test_loader,
#             device=device,
#             mode=config["mode"]
...
#                   f"Error={outlier_info['residuals'][i]:.3f}")
#         # Structural analysis
#         analyze_graph_structure_outliers(model, test_loader, device, mode)
...
# --------------------------------------------
OUTPUT_DIR = config.get("output_directory", "training_results")
...
def train_model(model, optimizer, early_stopping, train_loader, val_loader, test_loader, fold_idx=None):
    ...
    model_filename = f"graphsage_{mode}_{mask_strategy}_{evidence_type}{log_suffix}{fold_suffix}.pt"
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"\n{'='*60}")
    print(f"Starting GraphSAGE Training")
    print(f"Mode: {mode}")
