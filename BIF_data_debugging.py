...
# """
# GNN-Only Benchmark with Per-Graph Timing
# =========================================
# Runs GNN inference on all networks and saves:
# - Per-network predictions
# - Per-network inference timing
# - All metrics

# Output: gnn_results.json (compatible with unified benchmark)
# """
...
#         # Create graph feature tensor [20-22]
#         graph_feats = torch.tensor([
#             np.log(num_nodes + 1),
#             graph_density,
#             max_path / 10.0
...
# class ModelBenchmark:
#     """Benchmark with proper 25-feature handling + PER-GRAPH TIMING"""
#     def __init__(self, model_path: str, config_path: str = "config.yaml", device=None):
#         with open(config_path, "r") as f:
#             self.config = yaml.safe_load(f)
...
#     def predict_single_graph(self, data: Data) -> Tuple[float, float]:
#         """
#         Returns: (prediction, inference_time_seconds)
#         """
#         data = data.to(self.device)
#         data.batch = torch.zeros(data.x.size(0), dtype=torch.long, device=self.device)
...
#     def evaluate_dataset(self, graphs, metadata_list):
#         predictions, ground_truths, timings = [], [], []

#         for graph, meta in zip(graphs, metadata_list):
#             pred, elapsed = self.predict_single_graph(graph)
#             true = graph.y.item()
#             predictions.append(pred)
#             ground_truths.append(true)
...
#         # Add timing metrics
#         metrics["avg_time_ms"] = np.mean(timings) * 1000
#         metrics["median_time_ms"] = np.median(timings) * 1000
#         metrics["min_time_ms"] = np.min(timings) * 1000
#         metrics["max_time_ms"] = np.max(timings) * 1000
...
#                 "prediction_prob": pred_prob,
#                 "absolute_error": abs(pred_prob - true_prob),
#                 "inference_time_ms": timings[i] * 1000,  # ← NEW: Per-graph timing
#                 "use_log_prob": self.use_log_prob
#             })
...
# def main():
#     print("=" * 70)
#     print("GNN BENCHMARK WITH PER-GRAPH TIMING")
#     print("=" * 70)
...
#     # Also save to benchmark_results for backwards compatibility
#     with open(os.path.join(output_dir, "results.json"), "w") as f:
#         json.dump(results, f, indent=2)
...
#     print(f"\n✓ Saved to {output_dir}/")
#     print(f"✓ Saved GNN results to: {output_file} (for unified benchmark)")
#     print("=" * 70)
...
"""
GNN-Only Benchmark with Per-Graph Timing (FIXED)
=================================================
Runs GNN inference on all networks with proper warmup to avoid cold start issues.
- Per-network predictions with accurate timing
- Warmup to eliminate first-inference overhead
- All metrics in compatible format
...
class ModelBenchmark:
    """Benchmark with proper 25-feature handling + FIXED WARMUP"""
    def __init__(self, model_path: str, config_path: str = "config.yaml", device=None):
        with open(config_path, "r") as f:
            self.config = yaml.safe_load(f)
        ...
        # CRITICAL: Run warmup AFTER model is loaded
        self._warmup()
    ...
    def _warmup(self):
        """Warmup GPU/model to avoid cold start timing issues

        Important:
         - ensure dummy graphs include at least one root node (node_type==0)
         - don't let a warmup failure crash initialization
        """
        print("  Running warmup...", end="", flush=True)
        
        # sizes chosen to exercise small→medium→large graph paths
        dummy_sizes = [8, 32, 128]  

        for idx, size in enumerate(dummy_sizes):
            # Create dummy node features (size x 25)
            x = torch.randn(size, 25, dtype=torch.float32).to(self.device)
            # Ensure first feature column (node_type) exists and set at least one root (node 0)
            # Many of your pipelines place node_type at index 0: 0=root,1=intermediate,2=leaf
            x[:, 0] = 2.0  # default to 'leaf' (non-root)
            x[0, 0] = 0.0  # ensure node 0 is root

            # Create a safe number of edges (avoid huge complete graphs)
            # Use at most 3*size edges (reasonable connectivity) and ensure valid indices
            num_edges = max(1, min(size * 3, max(1, size * (size - 1) // 10)))
            src = torch.randint(0, size, (num_edges,), dtype=torch.long)
            dst = torch.randint(0, size, (num_edges,), dtype=torch.long)
            edge_index = torch.stack([src, dst]).to(self.device)

            dummy_data = Data(x=x, edge_index=edge_index)
            dummy_data.batch = torch.zeros(size, dtype=torch.long, device=self.device)

            # Run the forward a few times but catch exceptions so warmup doesn't crash
            try:
                with torch.no_grad():
                    # call model multiple times to populate kernels / caches
                    for _ in range(3):
                        _ = self.model(dummy_data)
            except Exception as e:
                # If model still complains, print a warning and continue
                print(f"\n    ⚠ Warmup graph size={size} failed: {e}")
                # continue to next dummy size instead of crashing
                continue
    ...
    def predict_single_graph(self, data: Data) -> Tuple[float, float]:
        """
        Returns: (prediction, inference_time_seconds)
        Now with accurate timing after warmup
        """
        data = data.to(self.device)
        data.batch = torch.zeros(data.x.size(0), dtype=torch.long, device=self.device)
        ...
        if self.device.type == "cuda":
            ...
            # CPU timing with high-precision counter
    ...
    def evaluate_dataset(self, graphs, metadata_list):
        ...
        for i, (graph, meta) in enumerate(zip(graphs, metadata_list)):
            ...
            # Progress indicator every 5 graphs
            if (i + 1) % 5 == 0:
                print(f"    Processed {i+1}/{len(graphs)} networks...")
        ...
        metrics["min_time_ms"] = np.min(timings) * 1000
        metrics["max_time_ms"] = np.max(timings) * 1000
        metrics["std_time_ms"] = np.std(timings) * 1000
        ...
        for i, (graph, meta) in enumerate(zip(graphs, metadata_list)):
            ...
            network_results.append({
                ...
                "prediction_prob": pred_prob,
                "absolute_error": abs(pred_prob - true_prob),
                "inference_time_ms": timings[i] * 1000,
                "use_log_prob": self.use_log_prob
            })
    ...
    def visualize_results(self, results, output_dir="benchmark_results"):
        ...
        truths = np.array([r["ground_truth_prob"] for r in per_network])
        preds = np.array([r["prediction_prob"] for r in per_network])
        timings = np.array([r["inference_time_ms"] for r in per_network])
        ...
        # Figure 3: Timing distribution (NEW)
        plt.figure(figsize=(10, 6))
        plt.hist(timings, bins=30, edgecolor='black', alpha=0.7, color='green')
        plt.xlabel('Inference Time (ms)', fontsize=12)
        plt.ylabel('Frequency', fontsize=12)
        plt.title('Inference Time Distribution (After Warmup)', fontsize=14, fontweight='bold')
        mean_time = np.mean(timings)
        median_time = np.median(timings)
        plt.axvline(mean_time, color='red', linestyle='--', label=f'Mean: {mean_time:.2f}ms')
        plt.axvline(median_time, color='orange', linestyle='--', label=f'Median: {median_time:.2f}ms')
        plt.legend()
        plt.grid(True, alpha=0.3, axis='y')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'timing_distribution.png'), dpi=300)
        plt.close()
...
def main():
    print("=" * 70)
    print("GNN BENCHMARK WITH PROPER WARMUP")
    print("=" * 70)
    ...
    print(f"  Min:     {metrics['min_time_ms']:.2f}ms")
    print(f"  Max:     {metrics['max_time_ms']:.2f}ms")
    print(f"  Std Dev: {metrics.get('std_time_ms', 0):.2f}ms")
    
    # Check if timing is now consistent
    if metrics['max_time_ms'] > metrics['avg_time_ms'] * 10:
        print("\n⚠ WARNING: Large timing variance detected!")
        print("  Some networks still showing outlier times.")
        print("  Consider running benchmark again or checking for other issues.")
    else:
    ...
    print(f"\nTiming Analysis:")
    per_network = results["per_network_results"]
    sorted_by_time = sorted(per_network, key=lambda x: x["inference_time_ms"])
    
    print(f"\nFastest 3 networks:")
    for net in sorted_by_time[:3]:
        print(f"  {net['network_name']}: {net['inference_time_ms']:.2f}ms ({net['num_nodes']} nodes)")
