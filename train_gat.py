...
def debug_batch(data, batch_idx=0):
    ...
    # Debug function to check data structure and statistics
    print(" Feature shape:", data.x.shape)

    mean = data.x.mean(dim=0)
    std = data.x.std(dim=0)
    min_vals = data.x.min(dim=0).values
    max_vals = data.x.max(dim=0).values
    ...
    print(" Min:", min_vals)
    print(" Max:", max_vals)
    print(f" Unique feature rows: {unique_rows}/{data.num_nodes}")
