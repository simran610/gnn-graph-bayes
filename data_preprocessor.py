...
10-19 | CPD summary features  | Ten summary statistics derived from node CPD values:
                              | mean, std, min, max, entropy, argmax, count_nonzero, median, percentile_25, percentile_75
20-22 | Graph features        | Three auxiliary features computed per node(log graph size,graph density, normalised path length (max path lenght/10))
23    | evidence_strength     | Copy of evidence flag (per-node)
24    | distance_to_evidence  | Shortest path length to nearest evidence node

Important:
- Evidence flag is inserted at index 9 during preprocessing, shifting CPD summary start to index 10.
- Normalization applies to indices 1-8, 10-19 (except argmax), and 23.
- Masking applies to CPD summary feature columns (10-19).
- Graph level features (index 20-22).
- Log-probability training improves sensitivity to low probability ranges.
"""
...
class FeatureNormalizer:
    """Handles feature normalization with proper indexing"""
    
    def __init__(self, verbose=False):
        self.verbose = verbose
        self.stats = {}
        
    def fit(self, data_list, cpd_start_idx=10):
        """Compute normalization statistics from training data"""
        if self.verbose:
            print("Computing normalization statistics...")
        
        # Indices to normalize
        base_indices = [1, 2, 3, 4, 5, 6, 7, 8]  # degrees, centralities, variable_card, num_parents
        ...
        for data in data_list:
            x = data.x
            base_feats.append(x[:, base_indices].cpu().numpy())
            
            # CPD summary: skip argmax at index 5 (relative to cpd_start)
            cpd = x[:, cpd_start_idx:cpd_start_idx+10]
            cpd_to_norm = torch.cat([cpd[:, :5], cpd[:, 6:]], dim=1)  # Skip argmax
            cpd_feats.append(cpd_to_norm.cpu().numpy())
            
            dist_feats.append(x[:, -1:].cpu().numpy())
        
        # Compute stats
        base_all = np.vstack(base_feats)
        cpd_all = np.vstack(cpd_feats)
        dist_all = np.vstack(dist_feats)
        
        self.stats['base'] = {
            'mean': np.mean(base_all, axis=0),
            'std': np.std(base_all, axis=0) + 1e-8,
            'indices': base_indices
        }
        self.stats['cpd'] = {
            'mean': np.mean(cpd_all, axis=0),
            'std': np.std(cpd_all, axis=0) + 1e-8
        }
        self.stats['distance'] = {
            'mean': np.mean(dist_all),
            'std': np.std(dist_all) + 1e-8
        }
        
        if self.verbose:
            print(f"Normalization fitted on {len(data_list)} graphs")
            print(f"  Base features: mean={self.stats['base']['mean'][:3]}, std={self.stats['base']['std'][:3]}")
            print(f"  Distance: mean={self.stats['distance']['mean']:.4f}, std={self.stats['distance']['std']:.4f}")
    
    def transform(self, data, cpd_start_idx=10):
        """Apply normalization to a single graph"""
        x = data.x.clone()
        
        # Normalize base features
        indices = self.stats['base']['indices']
        x[:, indices] = (x[:, indices] - torch.tensor(self.stats['base']['mean'], dtype=torch.float)) / \
                        torch.tensor(self.stats['base']['std'], dtype=torch.float)
        
        # Normalize CPD (skip argmax)
        cpd = x[:, cpd_start_idx:cpd_start_idx+10]
        cpd_to_norm = torch.cat([cpd[:, :5], cpd[:, 6:]], dim=1)
        cpd_normed = (cpd_to_norm - torch.tensor(self.stats['cpd']['mean'], dtype=torch.float)) / \
                     torch.tensor(self.stats['cpd']['std'], dtype=torch.float)
        x[:, cpd_start_idx:cpd_start_idx+5] = cpd_normed[:, :5]
        x[:, cpd_start_idx+6:cpd_start_idx+10] = cpd_normed[:, 5:]
        
        # Normalize distance
        x[:, -1] = (x[:, -1] - self.stats['distance']['mean']) / self.stats['distance']['std']
    ...
    def load_stats(self, path):
        self.stats = torch.load(path, weights_only=True)
        if self.verbose:
...
class GraphPreprocessor:
    ...
    def __init__(self, mode="distribution", json_folder="graphs",
        ...
        self.bn_builder = BayesianNetworkBuilder(verbose=verbose)
        self.use_intermediate = use_intermediate
        self.use_log_prob = use_log_prob  # NEW: Log-probability flag
    ...
    def preprocess_graph(self, data, global_cpd_len, graph_idx=None):
        """Preprocess a single graph with consistent indexing and CPD summary features"""
        x = data.x.clone()

        num_nodes = x.size(0)

        # Size-normalize degree features (indices 1-2)
        x[:, 1] = x[:, 1] / num_nodes  # in_degree / total_nodes
        x[:, 2] = x[:, 2] / num_nodes  # out_degree / total_nodes
        ...
        if self.mode in ["distribution", "root_probability"]:
            ...
            num_nodes_meta = metadata.get("total_nodes", x.size(0))
            num_edges_meta = metadata.get("total_edges", 0)
            max_path_length = json_data.get("paths_info", {}).get("max_path_length", 0)

            # Calculate graph density
            if num_nodes_meta > 1:
                graph_density = num_edges_meta / (num_nodes_meta * (num_nodes_meta - 1))
            else:
                graph_density = 0.0
            ...
            # VERBOSE DEBUGGING  ##############
            if graph_idx < 3:  # Only for first 3 graphs
                print(f"\n{'='*60}")
                print(f"GRAPH {graph_idx} INFERENCE DEBUG")
                print(f"{'='*60}")
                print(f"Root node: {json_data['node_types']['roots']}")
                print(f"Leaf nodes count: {len(json_data['node_types']['leaves'])}")
                print(f"Intermediate nodes count: {len(json_data['node_types'].get('intermediates', []))}")
            ...
            if graph_idx < 3:
                ...
                print(f"Prob[0] (P(root=0)): {prob[0].item():.6f}")
                print(f"Prob[1] (P(root=1)): {prob[1].item():.6f}")
####################
            # DEBUG PRINTS:
                print(f"\n=== PREPROCESSING DEBUG (Graph {graph_idx}) ===")
                print(f"Raw prob from inference: {prob}")
                print(f"Prob type: {type(prob)}")
                print(f"Prob shape: {prob.shape}")
                print(f"use_log_prob: {self.use_log_prob}")


            # === NEW: Log-probability transformation ===
            if self.use_log_prob:
                # Add small epsilon to avoid log(0)
                eps = 1e-10
                prob_safe = torch.clamp(prob, min=eps, max=1.0)
                log_prob = torch.log(prob_safe)
                
                if self.mode == "distribution":
                    data.y = log_prob  # Log-probabilities for both classes
                else:  # root_probability
                    data.y = torch.tensor([log_prob[0].item()], dtype=torch.float)
            else:
                # Original probability-based targets
                if self.mode == "distribution":
                    data.y = prob
                else:  # root_probability
                    data.y = torch.tensor([prob[0].item()], dtype=torch.float)
        ...
        # --- Step 4: Compute graph-level auxiliary features ---
        
        log_num_nodes = np.log(num_nodes + 1)  # Log of size

        # Add graph size feature to ALL nodes
                # Graph-level features (same for all nodes in graph)
        graph_features = torch.tensor([
            np.log(num_nodes_meta + 1),  # Log size
            graph_density,                # Edge density
            max_path_length / 10.0,      # Normalized path length
        ...
        # Concatenate
        x = torch.cat([x, graph_features, evidence_strength, distance_to_evidence], dim=1)
        data.x = x
        ...
        if self.verbose:
            ...
            print(f"  Root CPD summary: {x[root_node, cpd_start_idx:cpd_start_idx + 10]}")
            print(f"  Evidence count: {(x[:, evidence_flag_idx] == 1).sum().item()}")
            print(f"  Target ({'log-prob' if self.use_log_prob else 'prob'}): {data.y}")
...
class DataPipeline:
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        self.verbose = self.config.get("verbose", False)
        self.use_intermediate = self.config.get("use_intermediate", False)
        self.use_log_prob = self.config.get("use_log_prob", False)  # NEW CONFIG OPTION
        ...
        if self.verbose:
            ...
            print(f"  Mask strategy: {self.mask_strategy}")
            print(f"  Use intermediate: {self.use_intermediate}")
            print(f"  Use log-probability: {self.use_log_prob}")
            print(f"  Num leaf to infer: {self.num_leaf_to_infer}")
            print(f"  Num intermediate to infer: {self.num_intermediate_to_infer}")
        ...
        self.preprocessor = GraphPreprocessor(
            ...
            num_leaf_to_infer=self.num_leaf_to_infer,
            num_intermediate_to_infer=self.num_intermediate_to_infer,
            use_log_prob=self.use_log_prob  # Pass log-prob flag
        )
        self.normalizer = FeatureNormalizer(verbose=self.verbose)
    ...
    def run_preprocessing_and_split(self):
        ...
        if use_kfold:
            ...
            if self.mode == "root_probability":
                if self.use_log_prob:
                    # For log-probs, stratify on the actual values (which are negative)
                    y_targets = np.array([float(g.y.item()) for g in processed])
                else:
                    y_targets = np.array([float(g.y.item()) for g in processed])
            elif self.mode == "distribution":
                if self.use_log_prob:
                    # Use log(P(class=1)) for stratification
                    y_targets = np.array([float(g.y[1].item()) for g in processed])
                else:
                    y_targets = np.array([float(g.y[1].item()) for g in processed])
            else:
            ...
            for fold_idx, (train_val_idx, test_idx) in enumerate(skf.split(np.arange(n_total), bins)):
                ...
                # Collect graph objects and normalize
                train_graphs_raw = [processed[i] for i in train_idx]
                self.normalizer.fit(train_graphs_raw, cpd_start_idx=10)

                train_graphs = [self.normalizer.transform(processed[i], cpd_start_idx=10) for i in train_idx]
                val_graphs = [self.normalizer.transform(processed[i], cpd_start_idx=10) for i in val_idx]
                test_graphs = [self.normalizer.transform(processed[i], cpd_start_idx=10) for i in test_idx]
                ...
                torch.save(val_graphs, f"datasets/folds/fold_{fold_idx}_val.pt")
                torch.save(test_graphs, f"datasets/folds/fold_{fold_idx}_test.pt")
                self.normalizer.save_stats(f"datasets/folds/fold_{fold_idx}_norm_stats.pt")
            ...
            self.normalizer.fit(processed[:train_end], cpd_start_idx=10)

            train = [self.normalizer.transform(g, cpd_start_idx=10) for g in processed[:train_end]]
            val = [self.normalizer.transform(g, cpd_start_idx=10) for g in processed[train_end:val_end]]
            test = [self.normalizer.transform(g, cpd_start_idx=10) for g in processed[val_end:]]
            
            os.makedirs("datasets", exist_ok=True)
            torch.save(train, "datasets/train.pt")
            torch.save(val, "datasets/val.pt")
            torch.save(test, "datasets/test.pt")
            self.normalizer.save_stats("datasets/norm_stats.pt")
            print(f"Classic split - Train: {len(train)}, Val: {len(val)}, Test: {len(test)}")
